#!/usr/bin/env python3
"""
Unit tests for magpiem.io.io_utils
...
class TestIOUtils:
    ...
    def test_read_emc_mat(self):
        ...
        try:
            ...
            # Check that values are arrays (particle data)
            for tomo_name, particles in result.items():
                assert hasattr(particles, "__len__")  # Check it's iterable
                if len(particles) > 0:
                    # Check that each particle has the expected structure
                    particle = particles[0]
                    assert hasattr(particle, "__len__")  # Check it's iterable
    ...
    def test_read_emc_tomogram(self):
        ...
        try:
            ...
            assert len(result.all_particles) > 0
