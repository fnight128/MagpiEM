...
def modify_emc_mat(
    tomo_dict: dict, out_path: str, inp_path: str, keep_selected: bool
):
    output_data = dict()
    for tomo_id in tomo_dict.keys():
        print(tomo_id)
        table_rows = list()
        try:
        ...
        if keep_selected:
            for particle_id in tomo_dict[tomo_id].selected_particle_ids():
                table_rows.append(mat_table[particle_id])
        else:
            for particle_id in tomo_dict[tomo_id].unselected_particle_ids():
                table_rows.append(mat_table[particle_id])
...
def read_emC(filename, cycle="cycle000", num_images=-1):
    """
    ...
    try:
        full_geom = scipy.io.loadmat(filename, simplify_cells=True)["tomoMeta"][
            cycle
        ][geom_key]
    except:
